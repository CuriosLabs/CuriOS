...
1. **Download the ISO**: from the [official GitHub repository](https://github.com/CuriosLabs/CuriOS/releases).
   ...
   ```bash
   wget --content-disposition https://github.com/CuriosLabs/CuriOS/releases/download/26.05.4/CuriOS_26.05.4_amd64_intel.iso
   ```

   Download and check ISO signature:

   ```bash
   wget --content-disposition https://github.com/CuriosLabs/CuriOS/releases/download/26.05.4/CuriOS_26.05.4_amd64_intel.iso.sha256
...
2. **Flash the ISO**: You can flash it to a USB drive using tools like [Etcher](https://etcher.balena.io/#download-etcher)
   ...
   ```bash
   caligula burn -s $(cat ./CuriOS_26.05.4_amd64_intel.iso.sha256)
   ```
   ...
   sudo dd if=CuriOS_26.05.4_amd64_intel.iso of=/dev/sda bs=10MB oflag=dsync status=progress
   ```
