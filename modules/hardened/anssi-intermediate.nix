...
{ config, lib, ... }: {
  # Declare options
  options = {
    curios.hardened.anssi.intermediate = {
      enable = lib.mkOption {
        type = lib.types.bool;
        default = true;
        description =
          "Intermediate hardening rules for a Linux system as recommended by ANSSI. Enable this after curios.hardened.anssi.minimal.enable";
      };
      rule8 = lib.mkOption {
        type = lib.types.bool;
        default = true;
  ...
  config = lib.mkIf config.curios.hardened.anssi.intermediate.enable {
    boot = {
      kernelParams =
        lib.optionals config.curios.hardened.anssi.intermediate.rule8 [
          "init_on_alloc=1"
          "init_on_free=1"
          "slab_nomerge=yes"
          "slub_debug=FZP"
      ...
      kernel.sysctl =
        (lib.optionalAttrs config.curios.hardened.anssi.intermediate.rule9 {
        ...
        // (lib.optionalAttrs config.curios.hardened.anssi.intermediate.rule12 {
        ...
        // (lib.optionalAttrs config.curios.hardened.anssi.intermediate.rule13 {
          "net.ipv6.conf.default.disable_ipv6" = 1;
          "net.ipv6.conf.all.disable_ipv6" = 1;
        })
        // (lib.optionalAttrs config.curios.hardened.anssi.intermediate.rule14 {
          "fs.suid_dumpable" = 0;
