...
# Platform compatibility test for aarch64-linux (RPI4 / RPI5)
...
let
  ...
  # Common CuriOS options (all enabled)
  curiosAllOptions = {
    desktop = {
      basics.enable = true;
  ...
  # Build a NixOS configuration for a given platform
  mkNixosConfig = platformModule: platformName:
    import <nixpkgs/nixos> {
      system = "aarch64-linux";
      configuration = { config, pkgs, lib, ... }:
        {
          imports = commonModules ++ [ platformModule ];
        } // baseConfig // {
          curios = curiosAllOptions // {
            platform.${platformName}.enable = true;
          };
        };
    };

  nixosRpi4 = mkNixosConfig ../modules/platforms/rpi4.nix "rpi4";
  nixosRpi5 = mkNixosConfig ../modules/platforms/rpi5.nix "rpi5";
  ...
  checkPkgs = allPackages:
    let unavailablePkgs = builtins.filter isUnavailable allPackages;
    in map getName unavailablePkgs;

  rpi4Unavailable = checkPkgs nixosRpi4.config.environment.systemPackages;
  rpi5Unavailable = checkPkgs nixosRpi5.config.environment.systemPackages;
  ...
  reportText = ''
    ...
    RPI4: ${
      toString (builtins.length rpi4Unavailable)
    } packages are NOT available:
    ${builtins.concatStringsSep "\n  - " ([ "" ] ++ rpi4Unavailable)}

    RPI5: ${
      toString (builtins.length rpi5Unavailable)
    } packages are NOT available:
    ${builtins.concatStringsSep "\n  - " ([ "" ] ++ rpi5Unavailable)}

    ${if builtins.length rpi4Unavailable > 0 || builtins.length rpi5Unavailable
    > 0 then ''
      ERROR: These packages need platform guards
             (e.g., && config.curios.platform.amd64.enable)
